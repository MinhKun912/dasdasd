...
import {act_get_lkike, act_getPost_comment, act_getPost_succsess} from "../redux/action/actions";
import {GET_LIKE, POST_GET_SUCCSES} from "../redux/constances/actionTypes";
...
export const GET_COMMENT_SAGA = function* (action) {
    try {
        let listComment = yield call(get_list_comment, action.data)
...
export const get_like = function* (action) {
    try {
        let like = yield call(get_like, action.payload);
        yield put(act_get_lkike(GET_LIKE, like.data))
    } catch (err) {
        console.log("loi cmnr", err)
    }
}